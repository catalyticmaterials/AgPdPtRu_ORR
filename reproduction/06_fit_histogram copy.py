    ...
    for dist in ['gross','net']:
        ...
        if i == 0 and dist == 'gross':
            ...
            for j, ax in enumerate(ax):
                ax.set_xticks(range(len(bin_centers))[::3])
                ax.set_xticklabels(labels=[f'{s:.2f}' for s in bin_centers[::3]])
                ax.set_yticks(np.linspace(-0.3,0.0,4))
                ax.tick_params(axis='both', labelsize=16)
                ax.set(xlim=(range(len(bin_centers))[0],range(len(bin_centers))[-2]),ylim=(-0.4,0.025))
                ax.text(0.03, 0.03, fr'*{["OH","O"][j]} contributions', fontsize=20, transform=ax.transAxes, 
            ...
            fig.text(0.5, 0.015, r'$\Delta\mathrm{G}_{\mathrm{ads}}-\Delta\mathrm{G}^{\mathrm{Pt}}_{\mathrm{ads}}$ [eV]', fontsize=18, transform=text_ax.transAxes,ha='center') 
            fig.text(0.01, 0.5, r'Bin contribution [mA/cm$^2$]', rotation=90, fontsize=18, transform=text_ax.transAxes, va='center')
            plt.subplots_adjust(hspace=0.00,bottom=0.15,left=0.15,top=0.9,right=0.90)
            #plt.tight_layout()
            filename = f'misc/{dist}_LR_cont_poster.png'
